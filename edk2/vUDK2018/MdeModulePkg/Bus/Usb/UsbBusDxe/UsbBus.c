...
{
  UINT8 nFramePollingPeriod     = 1;

  UINT8 nMaxIsoPacketsPerFrame  = 1;
  UINT8 nIsoPacketsPerFrame     = 0;

  UINT8 nUrbByOneFrameTransfers;
  UINT8 nIsoPacketsForData      = 0;
  
  UINTN MaxDataTransferPerFrame;
  UINTN TransferRedunt          = 0;
  ...
  if(Dev->Speed == EFI_USB_SPEED_LOW) {
    return EFI_INVALID_PARAMETER;
  }

  /// @todo add support of super speed devices
  if(Dev->Speed == EFI_USB_SPEED_SUPER) {
  ...
  switch (Dev->Speed){
    case EFI_USB_SPEED_HIGH:
      nMaxIsoPacketsPerFrame = 8;
      //nFramePollingPeriod = pow(2, EpDesc->Desc.Interval-1);
      nFramePollingPeriod = 1;     
      break;
    case EFI_USB_SPEED_FULL:
      nMaxIsoPacketsPerFrame = 1;
      nFramePollingPeriod = 1;
    default:
      break;
  }
  ...
  /// @note important
  // we can transfer minimum = nIsoPacketsPerFrame 
  // and maximum = EFI_USB_MAX_ISO_BUFFER_NUM = 7 in an URB.
  // But in the USB specification EFI_USB_MAX_ISO_BUFFER_NUM = 
  // 255 for full speed devices and
  // 1024 for high speed devices.
  // and
  // Because UEFI interface can't utilitize an endpoint with
  // poll rate faster than 1ms
  // so, for now
  if(nFramePollingPeriod != 1) {
    Status = EFI_UNSUPPORTED;
    goto ON_EXIT;
  }
  nMaxIsoPacketsPerFrame = 1;
  
  nIsoPacketsPerFrame = nMaxIsoPacketsPerFrame / nFramePollingPeriod;
  MaxDataTransferPerFrame = nIsoPacketsPerFrame * EpDesc->Desc.MaxPacketSize;

  ///@ todo add check of nIsoPacketsForData.
  // The number of packets you send in the URB must be a
  // multiple of the number of packets in each frame.
  nIsoPacketsForData = DataLength / EpDesc->Desc.MaxPacketSize;

  nUrbByOneFrameTransfers = DataLength / MaxDataTransferPerFrame;

  // first transfer full frame data
  for(UINT8 i = 0; i < nUrbByOneFrameTransfers; i++) {
    curDataPtr = (VOID*)((UINT8*)Data + i * MaxDataTransferPerFrame);
  ...
  TransferRedunt = DataLength % MaxDataTransferPerFrame;
  if(TransferRedunt != 0) {

    VOID* Buf = AllocateZeroPool (MaxDataTransferPerFrame);
    if (Buf == NULL) {
    ...
    if (EFI_ERROR (Status)) {
      ...
      // mem copy from Buf to Data
      CopyMem(( (UINT8*)Data + nUrbByOneFrameTransfers * MaxDataTransferPerFrame ), Buf, TransferRedunt);
    }
...
{
  // #1 Get EP Interval and
  // #1.1 set numberOfPackertsPerFrame
  // #1.2 set isochPacketSize 
  // #2 Calc numberOfPackets DataLength/ioschPacketSize
  // #3 set circle to transfer all DataLength because
  // "The isochronous transfer is designed to be completed within one USB frame time".
  
  // Data is transferred every nFramePollingPeriod bus interval
  UINT8 nFramePollingPeriod     = 1;
